	...
	for (auto& entry : mediaMap)
	{
		...
		auto fetchMediaTask = FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, mediaToCook, platform] {
			...
			if (mediaToCook->Language.Len() > 0)
			{
				...
				assetPackagePath = ObjectTools::SanitizeObjectPath(FPaths::Combine(sharedThis->localizedPackagePath, mediaToCook->Language, AkUnrealHelper::MediaFolderName));
			}
			else
			{
				assetPackagePath = ObjectTools::SanitizeObjectPath(FPaths::Combine(sharedThis->basePackagePath, AkUnrealHelper::MediaFolderName));
			}
